...
import type { WorkflowDefinition, CondEdgeDef, FanOutDef } from "../models/workflow-def.js";
  ...
  const builder: WorkflowBuilder = {
    ...
    subgraph(name: string, childDef: WorkflowDefinition) {
      ...
      for (const condEdge of childDef.condEdges ?? []) {
        state.condEdges.push({
          nodeName: `${name}.${condEdge.nodeName}`,
          from: `${name}.${condEdge.from}`,
          targets: condEdge.targets.map((t) => `${name}.${t}`),
          fn: condEdge.fn,
        });
      }
      ...
      for (const fanOut of childDef.fanOuts ?? []) {
        state.fanOuts.push({
          nodeName: `${name}.${fanOut.nodeName}`,
          from: `${name}.${fanOut.from}`,
          templateNode: `${name}.${fanOut.templateNode}`,
          fn: fanOut.fn,
        });
      }
