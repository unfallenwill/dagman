...
describe('extractVarRefs', () => {
  ...
  it('should extract multiple different references', () => {
    const refs = extractVarRefs('Build {{global.project}} using {{setup.tool}} and {{config}}')
  ...
  it('should handle code with template literals', () => {
    const refs = extractVarRefs('const x = `${process.env.HOME}`; Use {{global.key}}')
    expect(refs).toHaveLength(1)
...
describe('renderTemplate', () => {
  ...
  it('should render global reference', () => {
    const { text, missing } = renderTemplate('Deploy to {{global.env}}', (source, key) => {
  ...
  it('should render node reference', () => {
    const { text, missing } = renderTemplate('Use {{build.output}}', (source, key, nodeName) => {
      if (source === 'node' && nodeName === 'build' && key === 'output') return '/dist'
      return undefined
    })
  ...
  it('should report missing variables', () => {
    const { text, missing } = renderTemplate('Path: {{missing-key}}', () => undefined)
