...
import { workflow } from '../../src/api/workflow.js'
...
describe('workflow builder', () => {
  it('builds a simple linear workflow', () => {
    const a = node((_state: any) => {}, 'output')
    const b = node((_state: any) => {})

    const def = workflow('test', { state: {} }).add('a', a).add('b', b).edge('b', 'a').build()
  ...
  it('includes condEdge in the definition', () => {
    const classify = node((_state: any) => {}, 'intent')
    const tool = node((_state: any) => {}, 'answer')
    const chat = node((_state: any) => {})

    const routeFn = (state: any) => (state.intent === 'need_tool' ? 'tool' : 'chat')
  ...
  it('builds with multiple edges', () => {
    const a = node((_state: any) => {})
    const b = node((_state: any) => {})
    const c = node((_state: any) => {})
  ...
  it('preserves function references in NodeDef', () => {
    const fnA = (_state: any) => {
      console.log('A')
    }
    const fnB = (_state: any) => {
      console.log('B')
    }
    ...
    const def = workflow('fns', { state: {} }).add('a', a).add('b', b).build()
...
describe('fanOut builder', () => {
  it('includes fanOut in the definition', () => {
    const source = node((_state: any) => {}, 'items')
    const process = node((_state: any) => {}, 'result')
  ...
  it('supports multiple fanOuts', () => {
    ...
    const def = workflow('multi-fanout', { state: {} })
      ...
      .add('b', b)
      .add('c', c)
      .fanOut('a', 'b', (_state: any) => [1, 2])
      .fanOut('b', 'c', (_state: any) => ['x', 'y', 'z'])
      .build()
...
describe('subgraph builder', () => {
  ...
  it('remaps child condEdges with prefix', () => {
    const routeFn = (_state: any) => 'step2a'
    const childDef = workflow('child', { state: {} })
      .add(
        'step1',
        node((_state: any) => {}),
    ...
    const parentDef = workflow('parent', { state: {} }).subgraph('process', childDef).build()
  ...
  it('supports nested subgraphs', () => {
    ...
    const outerDef = workflow('outer', { state: {} }).subgraph('mid', middleDef).build()
